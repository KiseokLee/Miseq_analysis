---
title: "220728_Soil_pH_adaptation"
author: "Kiseok Lee"
date: "2022-07-28"
...
ggplot(df_phy_abun, aes(x=Sample_name, y=Total, fill=Replicate)) +  ## I put fill=value which was wrong
  ...
  ggtitle("Sequence Depth per sample (After merging pairs)")+
  mytheme_2d +
...
tax_table(phy.un)
length(rownames(otu_table(phy.un))) ##  1754 unassigned taxa
...
phy  ## 6343
phy.clean ## 4589
sum(otu_table(phy.clean)) # 150012
...
length(rownames(otu_table(phy.phylumNA))) ##  529 unassigned taxa

phy.clean <- pop_taxa(phy.clean, c(vec.phylumNA))
phy  ## 6343
phy.clean ## 4060
sum(otu_table(phy.clean)) # 148248
...
phy.clean.neg  ### 4055 otu <- 4060
sum(otu_table(phy.clean.neg))  ### 147977 <- 1580210
...
df_phy.clean_abun <- df_phy.clean_abun %>% left_join(df_phy_neg, by = c("SampleID"="SampleID"))
...
length(v.less5)   ## 1396
length(v.less10)  ## 2152
length(v.less20)  ## 2789
length(v.less30)  ## 3136
length(v.less40)  ## 3321
length(v.less50)  ## 3459

## execute
phy  ##  6343 singleton removed
phy.clean.neg ## 4055 negative, unassigned taxa (kingdom, phylum) removed.
# phy.clean.ss   ## 390 otu
(phy.clean.neg.5 <- pop_taxa(phy.clean.neg, v.less5))   # 2659
(phy.clean.neg.10 <- pop_taxa(phy.clean.neg, v.less10)) # 1903
(phy.clean.neg.20 <- pop_taxa(phy.clean.neg, v.less20)) # 1266
(phy.clean.neg.30 <- pop_taxa(phy.clean.neg, v.less30)) # 919
(phy.clean.neg.40 <- pop_taxa(phy.clean.neg, v.less40)) # 734
(phy.clean.neg.50 <- pop_taxa(phy.clean.neg, v.less50)) # 596

sum(otu_table(phy)) ## 154431 otu reads.
sum(otu_table(phy.clean)) ## 148248
sum(otu_table(phy.clean.neg)) ## 147977
sum(otu_table(phy.clean.neg.5)) ## 144079
sum(otu_table(phy.clean.neg.10)) ## 139157
sum(otu_table(phy.clean.neg.20)) ## 130207
...
df.phylum <- phy.m %>%
  ...
  psmelt()
df.phylum$Sample <- factor(df.phylum$Sample, levels = lab)
...
ggplot(df.phylum.rel, aes(x=Sample, y = RelAbundance, fill = Phylum)) + 
...
df.class <- phy.m %>%
  ...
  psmelt()
df.class$Sample <- factor(df.class$Sample, levels = lab)
...
ggplot(df.class.rel, aes(x=Sample, y = RelAbundance, fill = Class)) + 
  geom_bar(stat="identity", width = 0.8, position = 'stack', colour="black") +
  #scale_fill_discrete() +
  scale_fill_manual(values = my_color_collection) +
...
df.order <- phy.m %>%
  ...
  psmelt()
df.order$Sample <- factor(df.order$Sample, levels = lab)
...
df.order.rel[df.order.rel$RelAbundance < 1,]$Order <- 'Low abundance'
...
## 2. plot relative abundance
ggplot(df.order.rel, aes(x=Sample, y = RelAbundance, fill = Order)) + 
  geom_bar(stat="identity", width = 0.8, position = 'stack', colour="black") +
  #scale_fill_discrete() +
  scale_fill_manual(values = my_color_collection) +
...
df.family <- phy.m %>%
  ...
  psmelt()
df.family$Sample <- factor(df.family$Sample, levels = lab)
...
## 2. plot relative abundance
ggplot(df.family.rel, aes(x=Sample, y = RelAbundance, fill = Family)) + 
  geom_bar(stat="identity", width = 0.8, position = 'stack', colour="black") +
  #scale_fill_discrete() +
  scale_fill_manual(values = my_color_collection) +
...
df.genus <- phy.m %>%
  ...
  psmelt()
df.genus$Sample <- factor(df.genus$Sample, levels = lab)
...
## 2. plot relative abundance
ggplot(df.genus.rel, aes(x=Sample, y = RelAbundance, fill = Genus)) + 
  geom_bar(stat="identity", width = 0.8, position = 'stack', colour="black") +
  #scale_fill_discrete() +
  scale_fill_manual(values = my_color_collection) +
...
df.genus.rel.positive = df.genus.rel %>% filter(Sample == "Positive")

ggplot(df.genus.rel.positive, aes(x=Sample, y = RelAbundance, fill = Genus)) + 
  geom_bar(stat="identity", width = 0.8, position = 'stack', colour="black") +
  #scale_fill_discrete() +
  scale_fill_manual(values = my_color_collection) +
...
## Let's coarse-grain to see if there is a difference
- First, genus level
```{r}
## group by genus level
phy.clean.neg.10

psmelt_genus <- psmelt(phy.clean.neg.10)
df_genus <- psmelt_genus %>% group_by(Sample, Sample_name, ReplicateID, Replicate, Genus) %>% summarize(Abundance = sum(Abundance))

# 37 samples total
df_genus$ReplicateID %>% unique()
length(df_genus[is.na(df_genus)])

## draw venn diagram
venn_each_sample_genus <- function(phy.clean.neg.10){
  library(VennDiagram)
  df_subset <- psmelt(phy.clean.neg.10) %>%  select(Genus, Sample, Replicate, Abundance) %>% group_by(Sample) %>%  filter(Abundance > 0) %>% arrange(Sample)
  df_Genus_1 <- df_subset %>% filter(Replicate == 1) %>% select(Genus) %>% unique()
  vec.f <- df_Genus_1$Genus
  df_Genus_2 <- df_subset %>% filter(Replicate == 2) %>% select(Genus) %>% unique()
  vec.s <- df_Genus_2$Genus
  df_Genus_3 <- df_subset %>% filter(Replicate == 3) %>% select(Genus) %>% unique()
  vec.t <- df_Genus_3$Genus
  n12 <- intersect(vec.f,vec.s)
  n23 <- intersect(vec.s,vec.t)
  n13 <- intersect(vec.f,vec.t)
  n123 <- intersect(intersect(vec.f,vec.s),vec.t)
...
for (i in 1:length(l_sample_names)){
  ...
  j <- l_sample_names[i]
  phy.ss1 <- subset_samples(phy.clean.neg.10, Sample_name == j)
  l_venn_genus[[i]] <- grid.arrange(gTree(children=venn_each_sample_genus(phy.ss1)), top = paste0(j))
}
length(l_venn_genus)

grid.arrange(grobs = l_venn_genus, ncol=5)


## Let's look at rank abundance
df_genus <- psmelt(phy.clean.neg.10) %>% group_by(Sample, Genus) %>% summarize(Abundance = sum(Abundance))

mat_genus <- df_genus %>% pivot_wider(names_from = Sample, values_from = Abundance)
mat_genus %>% tail()
dim(mat_genus)
...
mat_genus <-na.omit(mat_genus)
mat_genus
mat_genus <- tibble::column_to_rownames(mat_genus, var = "Genus")

mat_rel <- apply(mat_genus,2, function(x) x/sum(x))
dim(mat_rel) # 128 genus
colSums(mat_rel)

## Reorder the rows and change rownames
mat_rel2 <- mat_rel[names(sort(rowSums(mat_rel), decreasing = T)),,drop=F]
rownames(mat_rel2) <- paste0("genus", sprintf("%03d", 1:dim(mat_rel2)[1]))
...
mat_rank
mlt_rank <- melt(mat_rank)
colnames(mlt_rank) <- c("Genus", "Sample", "Rank")  ## changed to "Genus"
head(mlt_rank)

dim(mlt_rank) #70411 3
mlt_rank_meta <- mlt_rank %>% left_join(df_meta, by=c("Sample"="SampleID"))
dim(mlt_rank_meta) #4736 14
colnames(mlt_rank_meta)
...
# plot
## get the rank abundance aligned to Barneveld2_T0
vec_genus_B2T0 <- mlt_B2_T0 %>% filter(Replicate == 1) %>% arrange(Rank) %>% select(Genus)
dim(vec_genus_B2T0)
head(vec_genus_B2T0)
vec_genus_B2T0 <- vec_genus_B2T0[,1]
mlt_B2_T0$Genus <- factor(mlt_B2_T0$Genus, levels = vec_genus_B2T0)

mlt_B2_T0$Replicate <- factor(mlt_B2_T0$Replicate)　　　

ggplot(mlt_B2_T0 %>% filter(Replicate ==1), aes(x = Genus, y = Rank)) +
  # geom_line(aes(col=Sample)) +
  geom_point(aes(col=Replicate)) +
  ggtitle("Barneveld2_T0") +
  ylab("Rank \n") +
...
# what is happening here?
ggplot(mlt_B2_T0, aes(x = Genus, y = Rank)) +
  # geom_line(aes(col=Sample)) +
  geom_point(aes(col=Replicate)) +
  ggtitle("Barneveld2_T0") +
  ylab("Rank \n") +
...
## plot across different samples, align genus to replicate 1
l_sample_names <- mlt_rank_meta %>% select(Sample_name) %>% unique()
l_sample_names <- l_sample_names[,1]
l_sample_names <- l_sample_names[-length(l_sample_names)] # remove positive sample

plot_rank_genus <- function(mlt_rank_meta, Sample_name_i = "Barneveld2_T0", Replicate_i = 1){
  mlt_B2_T0 <- mlt_rank_meta %>% filter(Sample_name == Sample_name_i) 
  dim(mlt_B2_T0)
  head(mlt_B2_T0)
  
  ## get the rank abundance aligned to Barneveld2_T0
  vec_genus_B2T0 <- mlt_B2_T0 %>% filter(Replicate == Replicate_i) %>% arrange(Rank) %>% select(Genus)
  dim(vec_genus_B2T0)
  head(vec_genus_B2T0)
  vec_genus_B2T0 <- vec_genus_B2T0[,1]
  mlt_B2_T0$Genus <- factor(mlt_B2_T0$Genus, levels = vec_genus_B2T0)
...
for (i in 1:length(l_sample_names)) {
    print(i)
    l_plot[[i]] <- plot_rank_genus(mlt_rank_meta, l_sample_names[i], Replicate_i=1)
}
...
for (i in 1:length(l_sample_names)) {
    print(i)
    l_plot[[i]] <- plot_rank_genus(mlt_rank_meta, l_sample_names[i], Replicate_i=2)
}
...
for (i in 1:length(l_sample_names)) {
    print(i)
    l_plot[[i]] <- plot_rank_genus(mlt_rank_meta, l_sample_names[i], Replicate_i=3)
}
...
mlt_rel2 <- melt(mat_rel2)
colnames(mlt_rel2) <- c("Genus", "Sample", "Rel_abundance")
head(mlt_rel2)

# join two dataframes
mlt_rank_rel <- mlt_rank_meta %>% left_join(mlt_rel2, by = c("Genus"="Genus", "Sample"="Sample"))
colnames(mlt_rank_rel)
# plot Rank (x) - relative abundance (y) plots
l_sample_names
...
```{r}
### We can then load the biom file with phyloseq function import_biom. We extract the OTU table with OTU abundances and the taxonomy table from the resulting phyloseq object.
bac_phylo=import_biom("data/220726_Miseq_micro_v2/merged_OTU/OTU_table_final.biom")
...
dim(map)
# Merge biom data object with sample metadata + tree data(this is rooted!)
phy_tree = read_tree("data/220726_Miseq_micro_v2/merged_OTU/rooted_tree.nwk")
phy_mo <- merge_phyloseq(bac_phylo, map, phy_tree) # merged OTU

class(phy_mo)
phy_mo   ## 3773 otus

## changing rank names
na.omit(tax_table(phy_mo))

## No spiecies for OTU?
colnames(tax_table(phy_mo)) <- c("Kingdom", "Phylum", "Class",
                              "Order", "Family", "Genus")
...
sample_data(phy_mo)$Sample_name <- factor(sample_data(phy_mo)$Sample_name, levels =lab)
length(lab)
...
# Unassigned Kingdom level
unique(tax_table(phy_mo)[,'Kingdom']) ## only bacteria, then no need to exclude
phy.un <- subset_taxa(phy_mo, Kingdom == "Unassigned")
vec.un <- rownames(otu_table(phy.un))
tax_table(phy.un)
length(rownames(otu_table(phy.un)))  ##  3107 unassigned taxa
...
phy_mo.clean <- pop_taxa(phy_mo, c(vec.un))
phy_mo  ## 3773
phy_mo.clean ## 666
sum(otu_table(phy_mo.clean)) # 28230

# Unassigned Phylum level
phy_mo.phylumNA <- subset_taxa(phy_mo.clean, is.na(Phylum))
vec.phylumNA <- rownames(otu_table(phy_mo.phylumNA))
tax_table(phy_mo.phylumNA)
length(rownames(otu_table(phy_mo.phylumNA))) ##  662 unassigned taxa

phy_mo.clean <- pop_taxa(phy_mo.clean, c(vec.phylumNA))
phy_mo  ## 3773
phy_mo.clean ## 4
sum(otu_table(phy_mo.clean)) # 10
# checking procedure of whether the MT and CP otus are cleaned
# taxa_names(subset_taxa(phy, Order == "D_3__Rickettsiales"))  # before 66
# taxa_names(subset_taxa(phy, Order=="D_3__Chloroplast")) # before 34
...
# phy.test4 <- phy.two %>% psmelt() 
# phy.test4
tax_table(phy_mo.clean)[,colnames(tax_table(phy_mo.clean))] <- gsub(tax_table(phy_mo.clean)[,colnames(tax_table(phy_mo.clean))],pattern="[A-Z]_[0-9]__",replacement="")

#' sample_data(phy.clean)$SampleID <- factor(sample_data(phy.clean)$SampleID, levels =target_PAB)
tax_table(phy_mo.clean)


## let's look at the negative samples
phy_mo.clean.otu <- otu_table(phy_mo.clean)
head(phy_mo.clean.otu)
df.clean.otu <- data.frame(phy_mo.clean.otu)
dim(df.clean.otu)
df.clean.otu$total <- apply(df.clean.otu, 1, sum)
head(df.clean.otu)
df.clean.otu <- tibble::rownames_to_column(df.clean.otu, var = 'OTU')

sample_names(phy_mo.clean)
# how about we delete major taxa in the negative sequence?
negative <- df.clean.otu[,c('OTU',"SampleA53", "SampleA65")]
negative
negative$total <- apply(negative[,-1], 1, sum)
...
neg.otu
length(neg.otu) # 5 strain
tax_table(phy_mo)[neg.otu]
# library(xlsx)
# write.xlsx(tax_table(phy)[neg.otu], 'neg.otu_taxonomy.xlsx')

## checking 
## exclude all negative taxa
phy_mo.clean.neg <- pop_taxa(phy_mo.clean, negative_0$OTU)
phy_mo.clean.neg  ### 4055 otu <- 4060
sum(otu_table(phy_mo.clean.neg))  ### 147977 <- 1580210

## visualize how much negative strains were present in each sample

df_phy_mo

df_phy_mo.clean <- psmelt(phy_mo.clean)

df_phy_mo.clean_neg <- df_phy_mo.clean %>% group_by(SampleID) %>% subset(OTU %in% negative_0$OTU) %>% summarise(Negative_reads = sum(Abundance))
df_phy_mo.clean_abun <- df_phy_mo.clean %>% group_by(SampleID, Sample_name, Replicate, ReplicateID) %>% summarise(Total = sum(Abundance))
df_phy_mo.clean_abun <- df_phy_mo.clean_abun %>% left_join(df_phy_mo_neg, by = c("SampleID"="SampleID"))
df_phy_mo.clean_abun %<>% mutate(Usable_reads = Total - Negative_reads)

l_rep <- c('Barneveld2_T0_R1', 'Barneveld2_T0_R2', 'Barneveld2_T0_R3', 'Barneveld2_T9_H0_R1', 'Barneveld2_T9_H0_R2', 'Barneveld2_T9_H0_R3', 'Barneveld2_T9_H0_CHL_R1', 'Barneveld2_T9_H0_CHL_R2', 'Barneveld2_T9_H0_CHL_R3', 'Barneveld2_T9_OH10_R1', 'Barneveld2_T9_OH10_R2', 'Barneveld2_T9_OH10_R3', 'Barneveld2_T9_OH10_CHL_R1', 'Barneveld2_T9_OH10_CHL_R2', 'Barneveld2_T9_OH10_CHL_R3', 'LaBaghWoods1_T0_R1', 'LaBaghWoods1_T0_R2', 'LaBaghWoods1_T0_R3', 'LaBaghWoods1_T9_H0_R1', 'LaBaghWoods1_T9_H0_R2', 'LaBaghWoods1_T9_H0_CHL_R1', 'LaBaghWoods1_T9_H0_CHL_R2', 'LaBaghWoods1_T9_OH8_R1', 'LaBaghWoods1_T9_OH8_R2', 'LaBaghWoods1_T9_OH8_CHL_R1', 'LaBaghWoods1_T9_OH8_CHL_R2', 'PMA_Barneveld2_T0_R1', 'PMA_Barneveld2_T0_R2', 'PMA_Barneveld2_T9_H0_R1', 'PMA_Barneveld2_T9_H0_R2', 'PMA_Barneveld2_T9_H0_CHL_R1', 'PMA_Barneveld2_T9_H0_CHL_R2', 'PMA_Barneveld2_T9_OH10_R1', 'PMA_Barneveld2_T9_OH10_R2', 'PMA_Barneveld2_T9_OH10_CHL_R1', 'PMA_Barneveld2_T9_OH10_CHL_R2', 'Negative_1', 'Negative_2', 'Positive')


colnames(df_phy_mo.clean_abun)
df_melt_phy_mo_clean <- melt(df_phy_mo.clean_abun, id.vars = c("SampleID", 'Sample_name', "Replicate","ReplicateID", "Total"), variable.name = "Usable") 

df_melt_phy_mo_clean$ReplicateID <- factor(df_melt_phy_mo_clean$ReplicateID, levels = l_rep)

ggplot(df_melt_phy_mo_clean, aes(x=ReplicateID, y=value, fill=Usable)) +  ## I put fill=value which was wrong
  geom_bar(stat="identity") +
  # scale_fill_brewer(palette='Set2') +
  xlab("Sample") +
  ylab("Number of sequence reads \n") +
...
## remove negative samples
l_noneg <- l_rep[!(l_rep %in% c("Negative_1","Negative_2"))]
phy_mo.clean.neg <- subset_samples(phy_mo.clean.neg, ReplicateID %in% l_noneg)
# 37 samples
...
otu_table(phy_mo.clean.neg) %>% rowSums() %>% sort()

v.less5 <- rownames(otu_table(phy_mo.clean.neg)[otu_table(phy_mo.clean.neg) %>% rowSums() < 5])
v.less10 <- rownames(otu_table(phy_mo.clean.neg)[otu_table(phy_mo.clean.neg) %>% rowSums() < 10])
v.less20 <- rownames(otu_table(phy_mo.clean.neg)[otu_table(phy_mo.clean.neg) %>% rowSums() < 20])
v.less30 <- rownames(otu_table(phy_mo.clean.neg)[otu_table(phy_mo.clean.neg) %>% rowSums() < 30])
v.less40 <- rownames(otu_table(phy_mo.clean.neg)[otu_table(phy_mo.clean.neg) %>% rowSums() < 40])
v.less50 <- rownames(otu_table(phy_mo.clean.neg)[otu_table(phy_mo.clean.neg) %>% rowSums() < 50])
...
## execute
phy_mo  ##  6343 singleton removed
phy_mo.clean.neg ## 4055 negative, unassigned taxa (kingdom, phy_molum) removed.
# phy_mo.clean.ss   ## 390 otu
(phy_mo.clean.neg.5 <- pop_taxa(phy_mo.clean.neg, v.less5))   # 2659
(phy_mo.clean.neg.10 <- pop_taxa(phy_mo.clean.neg, v.less10)) # 1903
(phy_mo.clean.neg.20 <- pop_taxa(phy_mo.clean.neg, v.less20)) # 1266
(phy_mo.clean.neg.30 <- pop_taxa(phy_mo.clean.neg, v.less30)) # 919
(phy_mo.clean.neg.40 <- pop_taxa(phy_mo.clean.neg, v.less40)) # 734
(phy_mo.clean.neg.50 <- pop_taxa(phy_mo.clean.neg, v.less50)) # 596

sum(otu_table(phy_mo)) ## 154431 otu reads.
sum(otu_table(phy_mo.clean)) ## 148248
sum(otu_table(phy_mo.clean.neg)) ## 147977
sum(otu_table(phy_mo.clean.neg.5)) ## 144079
sum(otu_table(phy_mo.clean.neg.10)) ## 139157
sum(otu_table(phy_mo.clean.neg.20)) ## 130207
